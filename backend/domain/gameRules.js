...
function drawCard(state, player, deckType, events) {
    ...
    events.push(makeEvent(player, 'card_draw', { player: player.name, deckType, card: card.text }));
}
...
function applyCardEffect(state, player, card, events) {
    ...
    switch (action.type) {
        ...
        case 'advance_nearest': {
            ...
            if (nearest < player.position) player.cash += state.settings.passGoAmount || 200;
        ...
        case 'repairs': {
            ...
            player.cash -= action.perHouse * houses + action.perHotel * hotels;
            break;
        }
    }
...
function buyShares(state, player, payload, events) {
    ...
    const founder = state.players.find(p => p.userId === corp.founderId);
...
function issueDebt(state, player, payload, events) {
    ...
    const interestRate = positiveNumber(payload.interestRate, 'interestRate');
    ...
    player.debts.push({ id: `debt-${uuidv4()}`, principal: amount, interestRate, collateral, issueDate: new Date().toISOString() });
    player.cash += amount;
